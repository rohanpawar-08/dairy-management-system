...
/**
 * Service for generating monotonic, collision-safe payment receipt numbers.
...
export class PaymentNumberService {
  generatePaymentNumber(db: Database.Database, businessDate: string): string {
    const cleanDate = businessDate.replace(/-/g, '');
    const settingKey = `payment_counter_${cleanDate}`;
    ...
    return `PAY-${cleanDate}-${sequencePadded}`;
  }
}

export const paymentNumberService = new PaymentNumberService();
